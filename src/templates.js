...
export function gthumb(bg, accent, kind, dark) {
  ...
  function padsTh(a, m, c, t) { return Array.from({ length: 32 }, (_, n) => { const r = Math.floor(n / 8), i = n % 8; const on = (r === 2) || (r === 0 && i % 4 === 0) || (r === 1 && (i === 2 || i === 6)); return `<rect x="${24 + i * 19}" y="${24 + r * 20}" width="14" height="14" rx="3" fill="${on ? a : c}" stroke="${m}"/>`; }).join(''); }
